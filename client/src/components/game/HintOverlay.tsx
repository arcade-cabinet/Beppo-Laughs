  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh ref={meshRef} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
        ...
        <meshBasicMaterial color="#ff2266" transparent opacity={0.7} side={THREE.DoubleSide} />
      </mesh>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, 0]}>
        ...
        <meshBasicMaterial color="#ff66aa" transparent opacity={0.3} side={THREE.DoubleSide} />
      </mesh>
    </group>
  );
}

function HandprintOnWall({
  position,
  rotation,
  side,
}: {
  ...
  return (
    <group>
      {hints.footprints.map((fp, idx) => (
        <ClownFootprint key={`fp-${idx}`} position={fp.position} rotation={fp.rotation} />
      ))}
