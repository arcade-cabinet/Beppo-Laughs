...
export function ForkPrompt() {
  ...
  const getHandPosition = (direction: 'north' | 'south' | 'east' | 'west') => {
    switch (direction) {
      case 'north': return 'top-1/3 left-1/2 -translate-x-1/2';
      case 'south': return 'bottom-1/3 left-1/2 -translate-x-1/2';
      case 'east': return 'top-1/2 right-8 -translate-y-1/2';
      case 'west': return 'top-1/2 left-8 -translate-y-1/2';
    }
  };
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-40">
      {pendingFork.options.map((option, idx) => (
        <button
          key={option.nodeId}
        ...
        >
          <div className={`text-7xl ${getRotation(option.direction)}`}>
            🖐️
          </div>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
        ...
        <div className="text-pink-400 text-xl font-bold animate-pulse 
                        drop-shadow-lg" style={{ textShadow: '0 0 10px #ff69b4' }}>
          CHOOSE YOUR PATH!
        </div>
